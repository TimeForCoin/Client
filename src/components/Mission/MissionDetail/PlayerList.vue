    ...
    <div v-for="(item) in Players" class="list-item" :key="item.player.id">
      ...
      <div class="btn-pair" v-if="isPublisher == true && playerStatus == 'running'">
        <a-button
          class="btn"
          icon="smile"
          @click="finishTask(item.player.id, item.player.nickname)"
        >完成</a-button>
        <a-button
          class="btn"
          icon="frown"
          @click="failTask(item.player.id, item.player.nickname)"
        >失败</a-button>
      </div>
      <div class="btn-pair" v-else-if="isPublisher == true && playerStatus == 'wait'">
        <a-button
          class="btn"
          icon="smile"
          @click="acceptPlayer(item.player.id, item.player.nickname)"
        >同意</a-button>
        <a-button
          class="btn"
          icon="frown"
          @click="refusePlayer(item.player.id, item.player.nickname)"
        >拒绝</a-button>
      </div>
